...
public class XQNetworkStateReceiver extends BroadcastReceiver {
	...
	private static XQNetWorkUtil.NetType netType;
	private static ArrayList<XQNetChangeObserver> netChangeObserverArrayList = new ArrayList<XQNetChangeObserver>();
	...
	private static BroadcastReceiver getReceiver() {
		if (receiver == null) {
			receiver = new XQNetworkStateReceiver();
		}
		return receiver;
	}
	...
	public void onReceive(Context context, Intent intent) {
		receiver = XQNetworkStateReceiver.this;
		if (intent.getAction().equalsIgnoreCase(ANDROID_NET_CHANGE_ACTION) || intent.getAction().equalsIgnoreCase(ZW_ANDROID_NET_CHANGE_ACTION)) {
			if (!XQNetWorkUtil.isNetworkAvailable(context)) {
				...
				netType = XQNetWorkUtil.getAPNType(context);
	...
	private void notifyObserver() {
		...
		for (int i = 0; i < netChangeObserverArrayList.size(); i++) {
			XQNetChangeObserver observer = netChangeObserverArrayList.get(i);
			if (observer != null) {
	 ...
	 *
	 */
	public static void registerObserver(XQNetChangeObserver observer) {
		if (netChangeObserverArrayList == null) {
			netChangeObserverArrayList = new ArrayList<XQNetChangeObserver>();
		}
	 ...
	 *            observerKey
	 */
	public static void removeRegisterObserver(XQNetChangeObserver observer) {
		if (netChangeObserverArrayList != null) {
